...
namespace graphics {
	...
	void SSAO::initialize()
	{
		...
		std::uniform_real_distribution<float> kernelDistribution1(0.125f, 1.0f);

		m_numSamples = 16;
		m_samples = new math::Vec3f[m_numSamples];

		for (unsigned int i = 0; i < m_numSamples; i++) {
			float m = (float)(i + 1) / (float)m_numSamples;
			m *= m * 0.75f;
			m += 0.25f;
			m_samples[i] = math::Vec3f(kernelDistribution0(engine), kernelDistribution0(engine), kernelDistribution1(engine)).normalized() * m;
		}
		...
		m_shader.initialize();
		m_shader.loadShader("res/shaders/SSAO.vert", graphics::SHADERTYPE_VERTEX);
		m_shader.loadShader("res/shaders/SSAO.frag", graphics::SHADERTYPE_FRAGMENT);
		m_shader.link();
	...
	void SSAO::setFramebufferSize(unsigned int width, unsigned int height)
	{
		...
		formats.push_back(graphics::FORMAT_R8);
